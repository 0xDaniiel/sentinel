    ...
    const interval = setInterval(() => {
      setAssets(prev =>
        prev.map(asset => {
          ...
          const speedDeg = (asset.speed / 111000) * (TICK_INTERVAL / 1000) * 100;
          ...
          if (Math.abs(distLat) > SPAWN_RADIUS * 1.5 || Math.abs(distLng) > SPAWN_RADIUS * 1.5) {
            heading = (heading + 180) % 360;
            lat = asset.position.lat + Math.cos((heading * Math.PI) / 180) * speedDeg;
            lng = asset.position.lng + Math.sin((heading * Math.PI) / 180) * speedDeg;
          }
